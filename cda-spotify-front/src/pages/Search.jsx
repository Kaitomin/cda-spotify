          ...
          {musicList.map(music => (
            <div key={music.id} className="music-item-search bg-white rounded">
              <Link to={`/music/${music.id}`} className="text-decoration-none">            
                <img src={`${import.meta.env.VITE_CLOUDINARY_IMG_URL}/${music.imgUri}`} alt={music.title} className="music-image-search rounded-top object-fit-cover" width={100 + '%'} height={185 + 'px'} />
